...
import { StyleSheet, View, ScrollView, FlatList, ImageBackground, Image, Pressable } from 'react-native'
...
export default function OrdersScreen ({ navigation, route }) {
  ...
  const handleNextStatus = async (order) => {
    try {
      ...
      showMessage({
        message: `Order ${order.id} advanced to the next status.`,
        type: 'success',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
      ...
      showMessage({
        message: `Could not advance order ${order.id} to the next status. ${error}`,
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
  ...
  const renderOrder = ({ item }) => {
    ...
    const statusImage = getOrderImage(item.status)
    
    return (
      <ImageCard
        imageUri={statusImage}
